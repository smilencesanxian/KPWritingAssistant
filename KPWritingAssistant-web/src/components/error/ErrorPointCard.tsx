...
export default function ErrorPointCard({ errorPoint, onClick }: ErrorPointCardProps) {
  const config = getTypeConfig(errorPoint.error_type);
  const isFlagged = errorPoint.is_flagged;

  return (
    <button
      onClick={() => onClick(errorPoint.id)}
      className={`w-full flex items-center gap-3 py-3.5 px-4 text-left border-b border-neutral-100 last:border-b-0 transition-colors hover:bg-neutral-50 active:bg-neutral-100 ${
        isFlagged ? 'border-l-4 border-l-orange-400 pl-3' : ''
      }`}
    >
      {/* Left icon */}
      {isFlagged ? (
